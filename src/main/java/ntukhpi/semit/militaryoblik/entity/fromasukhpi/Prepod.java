...
public class Prepod {
    ...
    //дата рождения
    // В БД АСУ СП дня рождения нет, НО ЦЕ ОБОВЯЗКОВА ІНФОРМАЦіЯ Для ВіЙСЬКОВОГО ОБліКУ!!!
    @Column(name = "data_rozd", nullable = false)
    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
    private LocalDate dr;

    //В БД АСУ УПД ХПИ можна отримати дані про посаду, яку обіймає співробітник
